...
class RateLimiterIntegrationTest {
  ...
  private static final RedisContainer redis = new RedisContainer(REDIS_IMAGE)
    ...
    .withCreateContainerCmdModifier(cmd ->
      Objects.requireNonNull(cmd
          .withName("camila-gateway-redis-testing-%s".formatted(UUID.randomUUID()))
          .getHostConfig())
  ...
  void rateLimiterShouldEnforceLimitsReactive() {
    ...
    final var client = WebClient.create("http://localhost:%d".formatted(port));
    final var statusFlux = Flux.range(1, 100)
      .flatMap(_ -> client.get()
          .uri("/product-dev/api/products")
          .exchangeToMono(resp -> Mono.just(resp.statusCode().value()))
          .onErrorResume(_ -> Mono.just(500))
        , 20);

    statusFlux.collectList().timeout(Duration.ofSeconds(60))
