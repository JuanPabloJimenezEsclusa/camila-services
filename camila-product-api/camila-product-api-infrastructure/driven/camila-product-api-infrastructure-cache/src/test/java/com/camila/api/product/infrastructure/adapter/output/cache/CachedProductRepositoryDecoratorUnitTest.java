...
class CachedProductRepositoryDecoratorUnitTest {
  ...
  void shouldReturnProductFromCacheWhenCacheHitOccurs(final String internalId, final Product cachedProduct) {
    // Given
    when(this.cachePort.get(eq("findByInternalId"), eq(internalId), eq(Product.class)))
      .thenReturn(Mono.just(cachedProduct));
  ...
  void shouldFetchFromDelegateAndCacheWhenCacheMissOccurs(final String internalId, final Product dbProduct) {
    ...
    when(this.delegate.findByInternalId(internalId))
      .thenReturn(Mono.just(dbProduct));
    when(this.cachePort.put(eq("findByInternalId"), eq(internalId), eq(dbProduct)))
      .thenReturn(Mono.empty());
  ...
  void shouldGenerateCorrectCacheKeyForDifferentWeightsAndPagination(final AppliedWeights weights,
    ...
    // Given
    final var products = List.of(createProduct("P1", "Product 1"));
    when(this.cachePort.getList(eq("sortedProducts"), eq(expectedKeyPattern), eq(Product.class)))
  ...
  void shouldHandleExtremeWeightValues() {
    ...
    final long limit = 10L;
    final var products = List.of(createProduct("P1", "Product 1"));
    when(this.cachePort.getList(eq("sortedProducts"), eq("su:0.0:st:0.0:pm:0.0:ds:0.0:off:0:lim:10"), eq(Product.class)))
      .thenReturn(Mono.just(products));
  ...
  void shouldHandleMaximumWeightValues() {
    ...
    when(this.cachePort.getList(eq("sortedProducts"), eq(expectedKey), eq(Product.class)))
      .thenReturn(Mono.just(products));
