...
@DisplayName("[UT][ProductMongoAdapter] Product Mongo Adapter Unit Tests")
class ProductMongoAdapterUnitTest {
  ...
  private static Stream<Arguments> sortByMetricsWeightsParams() {
    return Stream.of(
      arguments(new AppliedWeights(0.5, 0.3, 0.1, 0.1), 0L, 10L,
        new ProductMongoEntity[]{createEntity("P1", "Product 1"), createEntity("P2", "Product 2")},
      ...
      arguments(new AppliedWeights(0.25, 0.25, 0.25, 0.25), 10L, 20L,
        new ProductMongoEntity[]{createEntity("P3", "Product 3")},
        new Product[]{createProduct("P3", "Product 3")}),
      arguments(new AppliedWeights(1.0, 0.0, 0.0, 0.0), 0L, 5L,
        new ProductMongoEntity[]{createEntity("P4", "Product 4"), createEntity("P5", "Product 5"), createEntity("P6", "Product 6")},
      ...
      arguments(new AppliedWeights(0.0, 1.0, 0.0, 0.0), 5L, 15L,
        new ProductMongoEntity[]{createEntity("P7", "Product 7")},
        new Product[]{createProduct("P7", "Product 7")}));
  }

  private static Stream<Arguments> appliedWeightsVariationsParams() {
    return Stream.of(
      arguments(new AppliedWeights(0.5, 0.3, 0.1, 0.1), 0L, 10L),
      arguments(new AppliedWeights(0.25, 0.25, 0.25, 0.25), 10L, 20L),
      arguments(new AppliedWeights(1.0, 0.0, 0.0, 0.0), 5L, 15L),
      arguments(new AppliedWeights(0.0, 0.0, 0.0, 1.0), 100L, 200L),
      arguments(new AppliedWeights(0.0, 0.0, 0.0, 0.0), 0L, 10L),
      arguments(new AppliedWeights(1.0, 1.0, 1.0, 1.0), 0L, 10L));
  }

  private static ProductMongoEntity createEntity(final String internalId, final String name) {
    return new ProductMongoEntity("id-" + internalId, internalId, name, "Category", 100,
  ...
  void shouldFindProductByInternalIdAndMapToDomainModel(final String internalId,
                                                        final ProductMongoEntity entity,
                                                        final Product expectedProduct) {
    // Given
    when(this.productMongoRepository.findByInternalId(internalId)).thenReturn(Mono.just(entity));
    when(this.mapper.toProduct(entity)).thenReturn(expectedProduct);

    // When & Then
    this.productMongoAdapter.findByInternalId(internalId).as(StepVerifier::create)
      .expectNext(expectedProduct)
      .verifyComplete();

    verify(this.productMongoRepository).findByInternalId(internalId);
    verify(this.mapper).toProduct(entity);
    verifyNoMoreInteractions(this.productMongoRepository, this.mapper);
    verifyNoInteractions(this.mongoOperations);
  }
  ...
  void shouldReturnEmptyWhenProductNotFoundByInternalId() {
    ...
    when(this.productMongoRepository.findByInternalId(internalId)).thenReturn(Mono.empty());
    ...
    verify(this.productMongoRepository).findByInternalId(internalId);
    verifyNoMoreInteractions(this.productMongoRepository);
    verifyNoInteractions(this.mapper, this.mongoOperations);
  }
  ...
  void shouldPropagateErrorWhenRepositoryFailsOnFindByInternalId() {
    ...
    when(this.productMongoRepository.findByInternalId(internalId))
  ...
  @DisplayName("Should sort products by metrics weights using aggregation and map to domain models")
  void shouldSortProductsByMetricsWeightsUsingAggregationAndMapToDomainModels(final AppliedWeights weights,
                                                                              final long offset, final long limit,
                                                                              final ProductMongoEntity[] entities,
                                                                              final Product[] expectedProducts) {
    // Given
    when(this.mongoOperations.aggregate(any(Aggregation.class), anyString(), eq(ProductMongoEntity.class)))
    ...
    // When & Then
    this.productMongoAdapter.sortByMetricsWeights(weights, offset, limit).as(StepVerifier::create)
      .expectNext(expectedProducts)
      .verifyComplete();

    verify(this.mongoOperations).aggregate(any(Aggregation.class), anyString(), eq(ProductMongoEntity.class));

    for (final var entity : entities) {
      verify(this.mapper).toProduct(entity);
    }

    verifyNoMoreInteractions(this.mongoOperations, this.mapper);
    verifyNoInteractions(this.productMongoRepository);
  }
  ...
  void shouldReturnEmptyFluxWhenNoProductsMatchSortingCriteria() {
    ...
    verifyNoInteractions(this.mapper, this.productMongoRepository);
  }

  @Test
  @DisplayName("Should propagate error when aggregation fails")
  void shouldPropagateErrorWhenAggregationFails() {
    // Given
    final var weights = new AppliedWeights(0.5, 0.3, 0.1, 0.1);
    final long offset = 0L;
    final long limit = 10L;
    final var expectedException = new RuntimeException("Aggregation error");
    when(this.mongoOperations.aggregate(any(Aggregation.class), anyString(), eq(ProductMongoEntity.class)))
  ...
  @ParameterizedTest
  @MethodSource("appliedWeightsVariationsParams")
  @DisplayName("Should execute aggregation with different weight combinations")
  void shouldExecuteAggregationWithDifferentWeightCombinations(final AppliedWeights weights, final long offset,
                                                               final long limit) {
    // Given
    when(this.mongoOperations.aggregate(any(Aggregation.class), anyString(), eq(ProductMongoEntity.class)))
      .thenReturn(Flux.empty());
  ...
  void shouldHandleLargeNumberOfProductsInSortingResults() {
    ...
    final var entities = Stream.iterate(1, n -> n + 1)
      ...
      .toArray(ProductMongoEntity[]::new);
  ...
  @Test
  @DisplayName("Should handle zero weights in sorting")
  void shouldHandleZeroWeightsInSorting() {
    // Given
  ...
  @Test
  @DisplayName("Should handle maximum weight values in sorting")
  void shouldHandleMaximumWeightValuesInSorting() {
    // Given
  ...
  @Test
  @DisplayName("Should handle zero offset and limit in sorting")
  void shouldHandleZeroOffsetAndLimitInSorting() {
    // Given
    final var weights = new AppliedWeights(0.5, 0.3, 0.1, 0.1);
    final long offset = 0L;
    final long limit = 0L;
  ...
  @Test
  @DisplayName("Should handle large offset and limit values in sorting")
  void shouldHandleLargeOffsetAndLimitValuesInSorting() {
    // Given
    final var weights = new AppliedWeights(0.5, 0.3, 0.1, 0.1);
  ...
  @Test
  @DisplayName("Should handle only sales units weight")
  void shouldHandleOnlySalesUnitsWeight() {
    // Given
    final var weights = new AppliedWeights(1.0, 0.0, 0.0, 0.0);
    final long offset = 0L;
    final long limit = 10L;
    final var entity = createEntity("P1", "Product 1");
    final var product = createProduct("P1", "Product 1");
    when(this.mongoOperations.aggregate(any(Aggregation.class), anyString(), eq(ProductMongoEntity.class)))
      .thenReturn(Flux.just(entity));
    when(this.mapper.toProduct(entity)).thenReturn(product);
  ...
  @Test
  @DisplayName("Should handle only stock weight")
  void shouldHandleOnlyStockWeight() {
    // Given
    final var weights = new AppliedWeights(0.0, 1.0, 0.0, 0.0);
    final long offset = 0L;
    final long limit = 10L;
    final var entity = createEntity("P1", "Product 1");
    final var product = createProduct("P1", "Product 1");
  ...
  @Test
  @DisplayName("Should handle only profit margin weight")
  void shouldHandleOnlyProfitMarginWeight() {
    // Given
    final var weights = new AppliedWeights(0.0, 0.0, 1.0, 0.0);
    final long offset = 0L;
    final long limit = 10L;
    final var entity = createEntity("P1", "Product 1");
    final var product = createProduct("P1", "Product 1");
  ...
  @Test
  @DisplayName("Should handle only days in stock weight")
  void shouldHandleOnlyDaysInStockWeight() {
    // Given
    final var weights = new AppliedWeights(0.0, 0.0, 0.0, 1.0);
    final long offset = 0L;
    final long limit = 10L;
    final var entity = createEntity("P1", "Product 1");
    final var product = createProduct("P1", "Product 1");
