...
class ProductRSocketAdapterITCase extends MongoContainerConfig {
  ...
  @BeforeAll
  static void setUpOnce(@Autowired final RSocketRequester.Builder builder, @LocalServerPort final Integer port)
  ...
  void findByInternalIdOk() {
    ...
    requester.route("products.request-response-findByInternalId").data(message).retrieveMono(Product.class)
      .as(StepVerifier::create).consumeNextWith(product -> {
