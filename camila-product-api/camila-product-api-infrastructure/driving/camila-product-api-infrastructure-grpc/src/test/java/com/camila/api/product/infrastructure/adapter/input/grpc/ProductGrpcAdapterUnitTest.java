...
class ProductGrpcAdapterUnitTest {
  ...
  void shouldReturnProductByInternalId() {
    ...
    final var grpcProduct = com.camila.api.product.infrastructure.adapter.input.grpc.Product.newBuilder()
    ...
    when(this.productGrpcMapper.toProduct(domainProduct)).thenReturn(grpcProduct);
  ...
  void shouldSortProductsByMetricsWeights(final Double salesUnits, final Double stock, final Double profitMargin,
    ...
    final var grpcProduct1 = com.camila.api.product.infrastructure.adapter.input.grpc.Product.newBuilder().setInternalId("1").build();
    final var grpcProduct2 = com.camila.api.product.infrastructure.adapter.input.grpc.Product.newBuilder().setInternalId("2").build();

    when(this.productUseCase.sortByMetricsWeights(requestParams)).thenReturn(Flux.just(product1, product2));
    when(this.productGrpcMapper.toProduct(product1)).thenReturn(grpcProduct1);
    when(this.productGrpcMapper.toProduct(product2)).thenReturn(grpcProduct2);
    ...
    verify(this.responseObserver, timeout(10_000L).times(2)).onNext(any());
    verify(this.responseObserver, timeout(10_000L)).onCompleted();
    verifyNoMoreInteractions(this.productUseCase);
  }
