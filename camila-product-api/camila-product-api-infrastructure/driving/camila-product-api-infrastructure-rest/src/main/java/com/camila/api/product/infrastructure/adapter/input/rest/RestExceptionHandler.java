...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  protected ResponseEntity<ProblemDTO> handleNotFound(final NotFoundException exception, final ServerHttpRequest request) {
    log.debug("NotFound", exception);

    return this.buildProblemResponse("/problems/not-found", HttpStatus.NO_CONTENT.name(), HttpStatus.NO_CONTENT,
      Optional.ofNullable(exception.getMessage()).orElse(HttpStatus.NO_CONTENT.getReasonPhrase()), request,
  ...
  protected ResponseEntity<ProblemDTO> handleIllegalArgument(final IllegalArgumentException exception,
    ...
    return this.buildProblemResponse("/problems/invalid-argument", HttpStatus.BAD_REQUEST.name(),
      HttpStatus.BAD_REQUEST,
      Optional.ofNullable(exception.getMessage()).orElse(HttpStatus.BAD_REQUEST.getReasonPhrase()), request,
      Collections.emptyMap());
  }
  ...
  protected ResponseEntity<ProblemDTO> handleConstraintViolation(final ConstraintViolationException exception,
                                                                 final ServerHttpRequest request) {
    log.debug("ConstraintViolation", exception);

    final Map<String, List<String>> errors = new HashMap<>();
    if (exception.getConstraintViolations() != null) {
      exception.getConstraintViolations().stream().filter(Objects::nonNull).forEach(cv -> {
        final String rawPath = cv.getPropertyPath() != null ? cv.getPropertyPath().toString() : "";
        final String field = this.extractFieldName(rawPath);
        errors.computeIfAbsent(field, _ -> new ArrayList<>()).add(cv.getMessage());
      });
    }

    return this.buildProblemResponse("/problems/validation", HttpStatus.EXPECTATION_FAILED.name(),
  ...
  protected ResponseEntity<ProblemDTO> handleProductServiceException(final Throwable exception,
                                                                     final ServerHttpRequest request) {
    log.error("ProductException", exception);

    return this.buildProblemResponse("/problems/internal", HttpStatus.INTERNAL_SERVER_ERROR.name(),
      HttpStatus.INTERNAL_SERVER_ERROR,
      Optional.ofNullable(exception.getMessage()).orElse(HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase()),
      request, Collections.emptyMap());
  }

  private ResponseEntity<ProblemDTO> buildProblemResponse(final String type, final String title,
                                                          final HttpStatus status, final String detail, final ServerHttpRequest request,
                                                          final Map<String, List<String>> errors) {
    final var problemDTO = new ProblemDTO().type(type).status(status.value()).title(title).detail(detail)
    ...
    return ResponseEntity.status(status).contentType(MediaType.parseMediaType(APPLICATION_PROBLEM_JSON))
      .body(problemDTO);
  }
