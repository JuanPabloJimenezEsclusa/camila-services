...
abstract class AbstractCachedProductDecoratorITCase {
  ...
  protected abstract ProductUseCase cachedDecorator();

  protected abstract ProductUseCase mockProductUseCase();
  ...
  void shouldNotCacheResultsForDifferentProductIds(final String scenario, final String productId) {
    ...
    // When & Then
    mockProductUseCase().findByInternalId(productId).as(StepVerifier::create).expectNext(product1).verifyComplete();
    mockProductUseCase().findByInternalId(differentId).as(StepVerifier::create).expectNext(product2).verifyComplete();
  ...
  void shouldNotCacheResultsForDifferentSortParameters(final String scenario, Map<String, String> requestParams) {
    ...
    // When & Then
    mockProductUseCase().sortByMetricsWeights(requestParams).as(StepVerifier::create).expectNext(product1).verifyComplete();
    mockProductUseCase().sortByMetricsWeights(differentParams).as(StepVerifier::create).expectNext(product2).verifyComplete();
  ...
  void shouldVerifyCacheEvictionTimeout() {
    ...
    mockProductUseCase().findByInternalId(productId).as(StepVerifier::create).expectNext(product).verifyComplete();
