...
abstract class AbstractCachedProductDecoratorITCase {

  protected abstract ProductRepository cachedDecorator();
  ...
  private static Stream<Arguments> productIdScenarios() {
    ...
    return Stream.of(
      ...
      Arguments.of(Named.of("Long numeric ID", "9999999999"))
    );
  }
  ...
  void shouldCacheSortByMetricsWeightsResults(final AppliedWeights weights, final long offset, final long limit) {
    ...
    // When & Then
    cachedDecorator().sortByMetricsWeights(weights, offset, limit).as(StepVerifier::create).expectNext(product).verifyComplete();
    cachedDecorator().sortByMetricsWeights(weights, offset, limit).as(StepVerifier::create).expectNext(product).verifyComplete();
  ...
  void shouldNotCacheResultsForDifferentProductIds(final String productId) {
    ...
    // When & Then
    mockProductRepository().findByInternalId(productId).as(StepVerifier::create).expectNext(product1).verifyComplete();
    mockProductRepository().findByInternalId(differentId).as(StepVerifier::create).expectNext(product2).verifyComplete();

    verify(mockProductRepository()).findByInternalId(productId);
    verify(mockProductRepository()).findByInternalId(differentId);
    verifyNoMoreInteractions(mockProductRepository());
  }
  ...
  void shouldNotCacheResultsForDifferentSortParameters(final AppliedWeights weights, final long offset, final long limit) {
    ...
    // When & Then
    mockProductRepository().sortByMetricsWeights(weights, offset, limit).as(StepVerifier::create).expectNext(product1).verifyComplete();
    mockProductRepository().sortByMetricsWeights(differentAppliedWeights, offset, limit).as(StepVerifier::create).expectNext(product2).verifyComplete();
  ...
  void shouldVerifyCacheEvictionTimeout() {
    ...
    mockProductRepository().findByInternalId(productId).as(StepVerifier::create).expectNext(product).verifyComplete();
