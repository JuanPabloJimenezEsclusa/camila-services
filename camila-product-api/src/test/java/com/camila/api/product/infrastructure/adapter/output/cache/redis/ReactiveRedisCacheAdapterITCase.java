...
@ContextConfiguration(classes = {RedisCacheConfig.class, RedisAutoConfiguration.class})
...
class ReactiveRedisCacheAdapterITCase {
  ...
  private static Stream<Arguments> scenarios() {
    return Stream.of(
      ...
      Arguments.of("wrong-type", "k2", "abc", Integer.class, false, null),
      Arguments.of("missing-key", "k3", SKIP_PUT, String.class, false, null)
    );
  }
  ...
  void shouldHandleGetPutScenarios(final String name,
                                   final String key,
    ...
    if (!Objects.equals(storedValue, SKIP_PUT)) {
      // When (put)
      adapter.put(cacheName, key, storedValue)
        .as(StepVerifier::create)
        .verifyComplete();
    }

    // Then (get)
    final var resultMono = adapter.get(cacheName, key, requestedType.asSubclass(Object.class));
    if (expectPresent) {
      resultMono
        .as(StepVerifier::create)
    ...
    if (expectedValue instanceof List<?> expectedList) {
      ...
      // When (putList)
      adapter.putList(cacheName, listKey, expectedList)
        .as(StepVerifier::create)
        .verifyComplete();
      // Then (getList)
      adapter.getList(cacheName, listKey, Integer.class)
        .as(StepVerifier::create)
