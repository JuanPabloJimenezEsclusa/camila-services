...
public abstract class CouchbaseContainerConfig {
  ...
  private static final String PRODUCTS_JSON = Objects
    .requireNonNull(CouchbaseContainerConfig.class.getClassLoader().getResource("scripts/couchbase/products.json"))
    .getPath();
  ...
  private static final CouchbaseContainer container = new CouchbaseContainer(COUCHBASE_IMAGE)
    ...
    .withCreateContainerCmdModifier(cmd -> Objects
      .requireNonNull(cmd.withName("camila-couchbase-testing-" + UUID.randomUUID()).getHostConfig())
  ...
  private static void createBucketContent(final String path, final String body) throws IOException {
    final var requestScope = new Request.Builder().header("Authorization", Credentials.basic(USERNAME, PASSWORD))
      .url(path).post(new FormBody.Builder().add("name", body).build()).build();
    try (final var response = new OkHttpClient().newCall(requestScope).execute()) {
  ...
  private static void loadBucketCollectionData(final Bucket bucket) throws IOException {
    ...
    final List<Map<String, Object>> products = new ObjectMapper()
      .readValue(Files.readAllBytes(productJsonFile.toPath()), new TypeReference<>() {
      });
    final var collection = bucket.scope(BUCKET_SCOPE).collection(BUCKET_COLLECTION);

    products.forEach(productMap -> collection.upsert(UUID.randomUUID().toString(), JsonObject.from(productMap)));
  }

  private static void initBucket() {
    final var clusterOptions = ClusterOptions.clusterOptions(container.getUsername(), container.getPassword())
      .environment(ClusterEnvironment.builder().disableAppTelemetry(true).build());

    try (final var cluster = Cluster.connect(container.getConnectionString(), clusterOptions)) {
      ...
      createBucketContent("%s/%s/collections".formatted(buildBasePath(bucket.name()), BUCKET_SCOPE),
        BUCKET_COLLECTION);
  ...
  private static String buildBasePath(final String bucketName) {
    return "%s://%s:%d/pools/default/buckets/%s/scopes".formatted("http", container.getHost(),
      container.getBootstrapHttpDirectPort(), bucketName);
  }
